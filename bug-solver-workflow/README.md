...
- `solve` — approval-gated activation surface. Requires `approved: true` and a `planPath` from precheck; rejects multi-bug/split-required plans, plans not preserving `editingAllowed=false`, dirty precheck/caller worktrees, and base-HEAD mismatches before any edit-capable phase. Later milestones will implement isolated worktree solving and bounded repairs behind this gate.
