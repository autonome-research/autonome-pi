...
`v0.11.6` has been committed/tagged/pushed (`2f4b2b9 Harden mission resume cursor and cancellation`). The current working tree is preparing `v0.11.7` strict validation-cursor hardening: passed milestone validation reports are reusable only when their trusted head, validation config/model/runtime fingerprint, artifacts, and exact validated feature/repair/no-change evidence verify. Legacy or ambiguous cursors are intentionally revalidated. Cancellation during command/adversarial validation aborts without synthesizing failed validation reports or repair features from the cancellation reason. This follows the fresh auto_trading resume where M0 was revalidated/repaired after M1 had already been reached, and a user cancellation during M0 validation produced a bogus must objection. Do not resume obsolete pre-reset auto_trading runs; use the fresh plan/run artifacts from the clean dogfood baseline.
