...
`v0.11.7` has been committed/tagged/pushed (`8bca7f0 Harden mission validation resume cursors`). The current working tree is preparing `v0.11.8` transient-lock merge hardening after auto_trading failed merging F2-002 because validation left an untracked integration `uv.lock` while F2-002 intentionally tracked `uv.lock` due to `pyproject.toml` dependency changes. The fix quarantines regular untracked transient lockfiles with artifact backup/hash before merge, restores on merge failure, and refuses to read/delete symlinks/special/oversized files. Passed milestone validation reports are reusable only when their trusted head, validation config/model/runtime fingerprint, artifacts, and exact validated feature/repair/no-change evidence verify. Legacy or ambiguous cursors are intentionally revalidated. Do not resume obsolete pre-reset auto_trading runs; use the fresh plan/run artifacts from the clean dogfood baseline.
