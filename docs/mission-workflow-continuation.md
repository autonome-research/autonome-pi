...
`v0.11.3` has been committed/tagged/pushed (`e648730 Stabilize mission repair resume ids`). The current working tree is preparing `v0.11.5` has been committed/tagged/pushed (`8535d64 Make mission handoff metadata runner-owned`). The current working tree is preparing `v0.11.6` resume/cancellation hardening: passed milestone validation reports act as durable resume cursors so resume skips completed milestones, validation reports include trusted heads, and cancellation during command/adversarial validation aborts without synthesizing failed validation reports or repair features from the cancellation reason. This follows the fresh auto_trading resume where M0 was revalidated/repaired after M1 had already been reached, and a user cancellation during M0 validation produced a bogus must objection. Do not resume obsolete pre-reset auto_trading runs; use the fresh plan/run artifacts from the clean dogfood baseline.
