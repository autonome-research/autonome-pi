...
## 2026-06-11 — Live dogfood #2: auto_trading operational hardening (first operationally_ready completion)

Mission `operational-hardening-clis-runbook-20260610` on `/home/velvet/auto_trading` completed at **operationally_ready** — the first mission above `contract_validated`. Final state independently verified in the integration worktree: 229 tests pass (was 198), ruff clean (was 11 errors), mypy clean (was 17 errors), three new operator CLIs (`auto-trading-data-health`, `auto-trading-ingest-once`, `auto-trading-paper-cycle --dry-run`) work offline, 159-line `docs/RUNBOOK.md`, six credential-gated external categories recorded as explicit skips in `operatorDx.externalChecksSkipped`. Strategic repair planner (first live use) clustered iteration-1 failures into four targeted repairs with correct failure classes; the adversarial validator caught real bugs (unbounded `--limit` fetch work, FRED smoke using an equity ticker, blank-alias credential mis-detection).

Findings and fixes from this dogfood:

1. **Planner emits prose paraphrases instead of contract assertion ids in feature.assertions, and spreads capability-dependent validation commands across milestones** (both would hard-fail activation). Caught at operator plan review; patched the plan (id mapping, single-milestone merge, alpaca credentialEnv, deliverables). Durable fix belongs in kernel design M2: a deterministic canonicalize-or-reject step on planner output at plan time.
2. **Validator-omitted local assertion rows caused unfixable validation thrash.** The strict assertionResults change made every scoped coverage row require an explicit validator pass; validators reliably verdict the 11 contract ids and reliably omit all 19 sentence-length local assertion ids (7 straight failed iterations across two runs; repairs cannot fix a reporting mismatch). Fixes: (a) validator assertionResults/objections ids now canonicalize against scoped local assertions via `canonicalLocalAssertionId` (same matcher as handoffs); (b) `buildCoverageReport` treats local rows as blocking only on an explicit validator fail — omission records a visible `unknown`, since locals are enforced at the strict-handoff layer; contract rows keep hard unverified-blocks semantics; (c) validator prompt documents the contract. Smoke: omitted locals → completion; explicitly failed local → blocks. Durable fix for the roadmap: short stable ids (`L1`-style) for local assertions in the next fingerprint schema rev.
3. **Operator cancel + patch + resume worked exactly as designed**: cooperative cancel file ended the doomed run (exit 130, no synthesized failures), the patched runner resumed, all 8 features and 7 repair commits skipped via trusted state, and revalidation passed. Stale `lastError` was archived as `lastResolvedError(mission_completed)`.

Mission branch `mission/operational-hardening-clis-runbook-20260610` awaits manual merge into `main`.
