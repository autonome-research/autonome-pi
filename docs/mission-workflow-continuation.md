...
`v0.11.1` has been committed/tagged/pushed (`1b2bbcf Harden mission workflow trusted resume`). The current working tree is preparing `v0.11.2` handoff/transient-artifact hardening: omitted untracked root `uv.lock` files are auto-cleaned only when no dependency manifest changed, workers are explicitly told that lockfiles are not generic junk, and failed workers emit compact diagnostics artifacts in addition to capped diffs. Do not resume obsolete pre-reset auto_trading runs; use the fresh plan/run artifacts from the clean dogfood baseline.
