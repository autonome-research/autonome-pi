...
`v0.11.2` has been committed/tagged/pushed (`5e3462d Harden mission workflow transient artifacts`). The current working tree is preparing `v0.11.3` repair-resume hardening: dynamic repair feature ids are content-addressed rather than sequential-only, and registry updates preserve prior completed records when a colliding feature id has a different fingerprint. This addresses the cancelled fresh auto_trading resume where a new validator pass reused `repair-M0-1-1` for a different repair after the original `repair-M0-1-1` had already committed Alpaca execution adapter work. Do not resume obsolete pre-reset auto_trading runs; use the fresh plan/run artifacts from the clean dogfood baseline.
