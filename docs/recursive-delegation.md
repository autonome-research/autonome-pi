...
- **`delegation-journal-model.mjs`**: exact manifest/event payload validation and inspection reducer composed with the existing budget reducers and headroom arithmetic. Journal capacity measures the **actual canonical encoded record including LF**. Ordinary admission leaves all remaining terminal slots; a terminal append consumes only its named slot (<=4096 bytes). Admission also leaves conservative bounded derived-projection growth space. Shell commands are **unsupported**, not silently admitted with zero reservations; command admission and its three-slot reservations belong to the next executor slice.
...
**Exact execution blocker:** `process-journal.mjs` retains whole-run groups and may prune gone groups on later reservations. `assertProcessGroupsStopped` is a whole-run recovery assertion, not proof of an individual invocation's complete worker/command group set, its clean execution outcome, or descendant settlement. Waiting ancestors/other lanes legitimately remain alive. This storage slice therefore provides **no launched join or success/partial settlement API**, rather than accepting a fabricated `groupsInactive:true`/`cleanExit:true` argument. The next executor slice must implement live invocation/command-owned group tracking, direct-exit versus group-drain proof, usage diagnostics and cancellation/exit classification, then review the corresponding result/join extension. Do not weaken this blocker to expose v3.
