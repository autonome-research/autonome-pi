...
The four foundation test files and the separate compatibility gate suite above now exist; their execution is recorded in the progress handoffs. The named Commit 2–4 test files below remain proposed and unrun. Foundation/compatibility success is not runtime approval. Never use `npm test`, `npm run test:dynamic`, or a wildcard containing paused successor work.
