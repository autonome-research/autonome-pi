...
The run artifact directory reserves its complete internal layout in every emission mode, including entries not used by the current mode: `workflow-spec.json`, `workflow-checkpoint.json`, `workflow-result.json`, `workflow-processes.json`, `workflow-harness-manifest.json`, `workflow-harness.mjs`, and the `phase-outputs/` and `delegation/` directories (the latter reserved before any recursive feature exposure). Atomic persistence also owns temporary siblings named `<target>.<token>.tmp` for the checkpoint, result, and process journal; phase-output temporary files stay inside the reserved directory. Generated artifacts that would use any reserved name receive a deterministic digest suffix.
