...
- Worker handoffs are strict JSON: missing/malformed handoffs, mismatched feature ids, unknown assertions, or `changedFiles` that disagree with git status/diff fail validation instead of being silently synthesized. The runner pre-creates a handoff skeleton with the exact `featureId`, and workers are explicitly allowed to complete with `changedFiles: []` when the inherited trusted codebase already satisfies the feature. Dynamically generated repair features use short content-addressed ids plus stored repair signatures so a resumed validation pass cannot reuse `repair-...-1` for a different repair and overwrite or duplicate prior completed repair work. Very narrow `safeName`-equivalent handoff feature-id canonicalization is accepted for harmless punctuation/whitespace drift while still rejecting genuinely different ids.
...
- The runner protects commit hygiene by excluding/removing common generated junk (`__pycache__`, `.pytest_cache`, `.venv`, `*.egg-info`, etc.) before staging and refusing junk in commits. Lockfiles are not generic junk; however, an omitted untracked root `uv.lock` is auto-cleaned before strict handoff validation only when no dependency manifest changed, and the cleanup is recorded as an audit artifact.
