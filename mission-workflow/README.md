...
- The runner protects commit hygiene by excluding/removing common generated junk (`__pycache__`, `.pytest_cache`, `.venv`, `*.egg-info`, etc.) before staging and refusing junk in commits. Lockfiles are not generic junk; however, an omitted untracked root `uv.lock` is auto-cleaned before strict handoff validation only when no dependency manifest changed, and the cleanup is recorded as an audit artifact. If an integration-worktree transient `uv.lock` would block merging a feature branch that intentionally tracks `uv.lock`, the runner quarantines regular-file contents into run artifacts with size/hash metadata before merge and restores them on merge failure; symlinks, special files, and oversized files are not read or deleted.
