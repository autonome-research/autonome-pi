...
{{handoffRel}}
The handoff JSON must include: featureId, completed, outcome, evidence, commandsRun[{command,exitCode}], issuesDiscovered, leftUndone, notesForValidator. Optional v3 arrays may include architecturalDecisions, assumptions, externalServiceAssumptions, operatorSteps, testsAdded, risksNotAddressed, broadcastNotes; each entry must be a short plain string, not an object. You may keep legacy changedFiles/assertionsAddressed fields if already present, but the runner derives actual changed files and assigned assertion coverage deterministically.
