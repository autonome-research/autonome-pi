...
The continuation runtime uses a TypeScript bridge plus content-versioned native imports so `/reload` cannot pair new handlers with an older cached `continuation-store.mjs`. Same-process reload coverage uses Pi's actual extension loader. This refreshes the continuation store itself; changes to other native `.mjs` dependencies may still require restarting Pi.
