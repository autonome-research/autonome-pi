...
- supervised progress reviews: new hosted background `dynamic_workflow` and `scripted_workflow` calls from a TUI or RPC context with an originating session ID are privately marked with `supervisionMode: "main-agent"`. The public launch argument `progressReviewIntervalMs` is an optional strict integer (60,000–86,400,000 ms) that the trusted runner records as immutable start metadata; it is not read from the compiled workflow spec or arbitrary caller-authored metadata. On reload/restart, an existing durable schedule keeps its cadence and check identity. Only for a newly created schedule does the explicit launch cadence win; when omitted, the host uses `PI_THREAD_PHASE_SUPERVISION_CHECK_MS`, then ten minutes. Restarting does not reset elapsed time; an overdue schedule becomes one pending review rather than a catch-up burst. The field is a launch cadence, not a timeout or intervention control, and is rejected for foreground/unhosted/legacy/resume-override calls.
  ...
  - The scheduler never classifies a worker as busy/stuck, kills or retries it, resumes it, launches successor work, or changes an existing schedule's cadence in response to a later workflow call. Background work keeps running when a review is overdue or the main agent is unavailable. A stale/dead owner can appear as diagnostic evidence only.
  ...
  - Supervision starts only in TUI and RPC session hosts. Print/JSON worker contexts do not recursively supervise. State is capped at 500 records and 512 KiB; unreadable, malformed, or over-limit state fails closed. The typed launch field is the only public cadence control; the initial release intentionally has no public post-launch cadence mutation/helper, per-run reschedule command, intervention tool, progress score, inactivity heuristic, or automatic recovery.
